...
const Main = () => {
    ...
    return (
        <div className="min-h-[calc(100vh-56px)] bg-gray-50 bg-[url('https://etherscan.io/assets/svg/patterns/waves.svg')] bg-repeat px-5 md:min-h-[calc(100vh-85px)] md:px-20">
            ...
            <div className="grid grid-cols-1 gap-10 py-4 md:grid-cols-3">
                {gasPriceData.map(({ label, gweiPrice, usdPrice, colorClass }) => (
                    <GasPriceCard key={label} label={label} gweiPrice={gweiPrice} usdPrice={usdPrice} colorClass={colorClass} isLoading={isLoading} />
                ))}
            </div>
        </div>
    );
